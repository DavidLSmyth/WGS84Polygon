...
public class WGS84PolygonGrid extends PolygonGridBase<WGS84Coordinate> {
	...
	/**
	 * Auxiliary method to generate grid points in a specified polygon in an array of arrays
	 * @return grid points generated in the polygon generated by this class in an array of double arrays [[lat1, long1],[lat2, long2], ... ] 
	 * @throws Exception
	 */
	public double[][] generateGridPointsAsArray() throws Exception{
		List<WGS84Coordinate> gridPoints = generateGridPoints();
		double[][] returnArray = new double[gridPoints.size()][];
		for(int gridPointIndex = 0; gridPointIndex < gridPoints.size(); gridPointIndex++) {
			returnArray[gridPointIndex] = new double[] {gridPoints.get(gridPointIndex).getLat().doubleValue(), gridPoints.get(gridPointIndex).getLng().doubleValue()};
		}
		return returnArray;
	}
